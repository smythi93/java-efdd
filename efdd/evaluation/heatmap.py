...
def heatmap(
    ...
    if values.shape[1] > max_display:
        ...
        feature_names = [
            *feature_names[: max_display - 1],
            f"{values.shape[1] - max_display + 1} other features",
        ]
        values = new_values
    ...
    for i in range(len(feature_names) - 1):
        feature_names[i] = (
            feature_names[i]
            .replace("SCALAR_PAIR:middle.py:3:yGEx", "middle.py:1 - y \u2265 x")
            .replace("LINE:middle.py:8", "middle.py:6 - executed")
        )
    ...
    cb.set_label("SHAP values", size=12, labelpad=-10)
