...
class JavaLocationsFinder(jast.JNodeVisitor, LocationsFinder):
    def get_locations(self, base_dir: str):
        if os.path.isfile(base_dir):
            base_dir = ""
        path = os.path.join(base_dir, self.file)
        tree = _TREE_CACHE.get(path)
        if tree is None:
            # Parsing dominates evaluation cost: the same source file is otherwise
            # re-parsed once per analysis object (thousands of times per bug). The
            # visit below only reads the tree, so caching it is safe.
            with open(path, "r") as fp:
                tree = jast.parse(fp.read())
