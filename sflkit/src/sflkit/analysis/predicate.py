...
class Comp(enum.Enum):
    ...
    def evaluate(self, x, y):
        # Java values may be null (-> Python None) and get paired with numbers by
        # ScalarPairFactory; ordering comparisons (<, <=, >, >=) then raise a
        # TypeError on None/cross-type operands. Treat such incomparable cases as
        # an unsatisfied predicate rather than aborting the whole analysis.
        try:
            if self == Comp.LT:
                return x < y
            elif self == Comp.LE:
